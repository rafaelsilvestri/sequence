...
class SequenceDatabaseSimulation extends Simulation {
  ...
  val scn1: ScenarioBuilder = scenario("Create sequence 'foo' with Database")
    .exec(http("Type foo")
      .post("/sequences/db/foo"))
    .pause(5)

  val scn2: ScenarioBuilder = scenario("Create sequence 'bar' with Database")
    .exec(http("Type bar")
      .post("/sequences/db/bar"))
    .pause(5)
