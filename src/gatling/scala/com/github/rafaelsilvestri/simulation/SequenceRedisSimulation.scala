...
class SequenceRedisSimulation extends Simulation {
  ...
  val scn1: ScenarioBuilder = scenario("Create sequence 'foo' with Redis")
    .exec(http("Type foo")
      .post("/sequences/redis/foo"))
    .pause(5)

  val scn2: ScenarioBuilder = scenario("Create sequence 'bar' with Redis")
    .exec(http("Type bar")
      .post("/sequences/redis/bar"))
    .pause(5)
